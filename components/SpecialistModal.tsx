...
const SpecialistList: React.FC<{ title: string; specialists: Specialist[] }> = ({ title, specialists }) => {
  if (!specialists || specialists.length === 0) {
    return (
        <div>
            <h4 className="text-md font-semibold text-sky-400 mt-4 mb-2">{title}</h4>
            <p className="text-sm text-slate-500 italic">No {title.toLowerCase()} found nearby for this center.</p>
        </div>
    );
  }
  return (
      ...
      <ul className="space-y-2">
        {specialists.map((specialist, index) => (
          <li key={index} className="text-sm p-3 bg-slate-700/50 rounded-md">
            ...
            <p className="text-slate-400 mt-1">{specialist.address}</p>
            {specialist.phone && (
    ...
    >
      ...
      >
        <div className="sticky top-0 bg-slate-800 p-6 z-10 flex justify-between items-center border-b border-slate-700">
          <div>
            <h2 className="text-2xl font-bold text-sky-300">Specialists Near</h2>
            <p className="text-slate-400 mt-1">{center.name}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6">
          {Object.entries(center.nearbySpecialists).length > 0 ? (
            ...
            <p className="text-slate-400 text-center py-4">No specialist information was found for this center.</p>
          )}
        </div>
      </div>
    </div>
